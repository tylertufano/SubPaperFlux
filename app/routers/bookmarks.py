...
@router.post("/bulk-publish", status_code=status.HTTP_202_ACCEPTED, dependencies=[Depends(csrf_protect)])
def bulk_publish_bookmarks(
    body: dict,
    current_user=Depends(get_current_user),
    session=Depends(get_session),
):
    ids = body.get("ids") or []
    config_dir = body.get("config_dir") or body.get("configDir")
    instapaper_id = body.get("instapaper_id") or body.get("instapaperId")
    ...
    if not isinstance(instapaper_id, str) or not instapaper_id.strip():
        raise HTTPException(status_code=400, detail="instapaper_id is required")
    tag_list: Optional[List[str]] = None
    if tags_raw is not None:
        if isinstance(tags_raw, str):
            tag_list = [t for t in [s.strip() for s in tags_raw.split(",")] if t]
            ...
            raise HTTPException(status_code=400, detail="tags must be a string or list of strings")
    id_strings = [str(i) for i in ids]
    stmt = select(Bookmark).where(Bookmark.owner_user_id == current_user["sub"], Bookmark.id.in_(id_strings))
    rows: List[Bookmark] = session.exec(stmt).all()
    found_ids = {row.id for row in rows}
    missing = [bid for bid in id_strings if bid not in found_ids]
    skipped: List[dict] = []
    enqueued: List[str] = []
    for row in rows:
        if not row.url:
            skipped.append({"id": row.id, "reason": "missing_url"})
            continue
        payload = {
        ...
        if row.feed_id:
            payload["feed_id"] = row.feed_id
        if row.published_at:
            payload["published_at"] = row.published_at.isoformat()
        job = Job(type="publish", payload=payload, owner_user_id=current_user["sub"])
        session.add(job)
        session.flush()
        enqueued.append(job.id)
    session.commit()
    return {
        "requested": len(id_strings),
