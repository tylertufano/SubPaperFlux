...
def test_resolve_user_from_token_uses_primary_token_when_aux_missing(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    payload: Dict[str, Any] = {"sub": "user-606"}
    userinfo_payload: Dict[str, Any] = {
        "name": "UserInfo Secondary",
    ...
    assert identity["name"] == "UserInfo Secondary"
    assert identity["email"] == "secondary@example.com"
    assert identity["groups"] == ["Team-C"]
    assert cfg.userinfo_endpoint == "https://idp.example.com/userinfo"
